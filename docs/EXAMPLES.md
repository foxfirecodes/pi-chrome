...
Strict CSP note: `chrome_snapshot` works via a packaged extension script; `chrome_evaluate` may still be blocked on pages that disallow `unsafe-eval`. `chrome_screenshot`, tab/navigation tools, and real input also work.
