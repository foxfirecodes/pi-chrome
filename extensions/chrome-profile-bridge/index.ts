...
- \`chrome_snapshot\` is injected as a packaged extension script and works on strict-CSP pages without \`'unsafe-eval'\`. \`chrome_evaluate\` still compiles caller-provided code in MAIN world, so pages with strict CSP (e.g. github.com, many bank/SaaS apps) can throw \`EvalError: ... 'unsafe-eval' is not an allowed source of script\`. If evaluation is blocked, inspect with \`chrome_snapshot\`/\`chrome_screenshot\` and drive with viewport-coordinate \`chrome_click\`/\`chrome_type\`/\`chrome_key\`. \`chrome_navigate\`, \`chrome_screenshot\`, \`chrome_tab\`, and Chrome input all keep working under any CSP.
...
2. Use \`chrome_snapshot\` before clicking/typing; pass \`uid\` over \`selector\`. The default snapshot is concise and includes structural layout/context around controls so generic labels like “Add” have meaning. Use \`query\` (or \`chrome_find\`) and \`mode: "forms" | "interactive" | "pageMap" | "text" | "changes" | "full"\` to zoom instead of asking for a giant dump. Use \`chrome_inspect(uid)\` for nearby text/actions/form context around one candidate.
3. \`includeSnapshot=true\` on click/type/fill/key to verify in one round trip; follow with \`chrome_snapshot({mode:"changes"})\` when you only need what changed.
...
8. Run /chrome doctor when in doubt about connectivity or capabilities.
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			...
			titleIncludes: Type.Optional(Type.String()),
			background: Type.Optional(
				Type.Boolean({ description: "If true, run silently in the background without focusing Chrome. Default false." }),
			),
			host: Type.Optional(Type.String()),
			port: Type.Optional(Type.Number()),
		}),
